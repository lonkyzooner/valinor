...
var AsyncLock__default = /*#__PURE__*/_interopDefaultLegacy(AsyncLock);
...
class OutgoingMessage {
    ...
    writeAuthenticated() {
        ...
        common.writeAuthenticated(this.encoder);
        return this;
    }
...
class MessageReceiver {
    ...
    apply(document, connection, reply) {
        ...
        switch (type) {
            ...
            case exports.MessageType.SyncReply:
                message.writeVarUint(exports.MessageType.Sync);
                this.readSyncMessage(message, document, connection, reply, type !== exports.MessageType.SyncReply);
                if (message.length > 1) {
                    if (reply) {
            ...
            case exports.MessageType.Awareness:
                this.logger.log({
                    direction: 'in',
                    type: exports.MessageType.Awareness,
                    category: 'Update',
                });
...
class Connection {
    ...
    constructor(connection, request, document, timeout, socketId, context, readOnly = false, logger) {
        ...
        this.logger = logger;
        this.lock = new AsyncLock__default["default"]();
     ...
     */
    send(message) {
        if (this.webSocket.readyState === common.WsReadyStates.Closing
            || this.webSocket.readyState === common.WsReadyStates.Closed) {
            this.close();
        }
        try {
    ...
    check() {
        if (!this.pongReceived) {
            return this.close(common.ConnectionTimeout);
        }
        if (this.document.hasConnection(this)) {
            ...
            try {
                ...
                this.close(common.ConnectionTimeout);
            }
        }
    }
    /**
    ...
    handleMessage(data) {
        this.callbacks.beforeHandleMessage(this.document, data)
            ...
            .catch((e) => {
            this.close({
                code: 'code' in e ? e.code : common.Forbidden.code,
                reason: 'reason' in e ? e.reason : common.Forbidden.reason,
            });
        });
    }
    /**
...
class Debugger {
    ...
    log(message) {
        ...
        const item = {
            ...message,
            type: exports.MessageType[message.type],
...
class Document extends Y.Doc {
    ...
    merge(documents) {
        (Array.isArray(documents) ? documents : [documents]).forEach(document => {
            Y.applyUpdate(this, Y.encodeStateAsUpdate(document));
        });
        return this;
    }
    /**
...
var module$1 = "dist/hocuspocus-server.esm.js";
...
class Hocuspocus {
    ...
    showStartScreen() {
        ...
        console.log();
        console.log(`  ${kleur__default["default"].cyan(`Hocuspocus v${meta.version}${name}`)}${kleur__default["default"].green(' running at:')}`);
        console.log();
        console.log(`  > HTTP: ${kleur__default["default"].cyan(`${this.httpURL}`)}`);
        ...
        console.log();
        console.log(`  ${kleur__default["default"].green('Ready.')}`);
        console.log();
    }
    /**
    ...
    closeConnections(documentName) {
        ...
        this.documents.forEach((document) => {
            ...
            document.connections.forEach(({ connection }) => {
                connection.close(common.ResetConnection);
            });
        });
    }
    /**
    ...
    async createDocument(documentName, request, socketId, connection, context) {
        ...
        document.awareness.on('update', (update) => {
            this.hooks('onAwarenessUpdate', {
                ...
                states: common.awarenessStatesToArray(document.awareness.getStates()),
            });
        });
        return document;
    }
